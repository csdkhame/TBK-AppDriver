...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(car_price) FROM  ".$dbname."  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' and driver_pickup > '0' and number_of_report >0 and car_price > 0 and drivername='".$driver_id."'  "; 
...
$arr[drivername] = $db->fetch($res[drivername]);
  ...
  $admin_company = $arr[companydriver][admin_company];
	...
	$find_date_today="and transfer_date BETWEEN '". $day_start."' AND '". $daywork."'";
...
//$row_no = $db->num_rows(TB_transfer_report_all,"id","drivername='".$driver_id."' and transfer_date='".$daywork."' and status = 'CONFIRM' and driver_pickup  < '1' group by driver_complete,invoice, order by driver_complete desc   ");
$row_yes = $db->num_rows("".$dbname."","id","drivername='".$driver_id."' $find_date and status = 'CONFIRM' and driver_pickup > '-1' and number_of_report >0 and car_price > 0 group by driver_complete,invoice   ");
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
  <tr  >
    <td  class="font_16" style="height:30px; padding-left:5px; ">ค่าเที่ยว เบี้ยเลี้ยง (ปกติ)</td>
    <td width="80" align="right"  class="font_20" style="padding-right:10px; color:#000000 "> 
  ...
  <tr  >
    <td  class="font_16" style="height:30px; padding-left:5px; ">ค่าเที่ยว เบี้ยเลี้ยง (2 เท่า)</span></td>
	...
	<span class="font_14">( <?=$pay_all-$pay_bonus?>x2 )&nbsp;</span><?= number_format( $pay_extra , 0 );?>
  ...
  <tr  style="display:none" >
    <td  class="font_16" style="height:30px; padding-left:5px; ">รวมค่าเที่ยว เบี้ยเลี้ยง</td>
       ...
       <td><table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
         <tbody>
           <tr>
             ...
             <td class="font_18" ><font color="#FF0004">&nbsp;วันที่มีค่าเที่ยว เบี้ยเลี้ยงคูณ 2 เท่า</td>
           </tr>
         </tbody>
       </table></td>
     </tr>
