...
<table width="100%"  border="0" cellspacing="2" cellpadding="2"  >
  <tr align="center">
    <td width="50" bgcolor="#999999" class="font_16" style="color:#FFFFFF; padding:2px; "><strong>วันที่</strong></td>
    <td width="40" bgcolor="#999999" class="font_16" style="color:#FFFFFF; padding:2px; "><strong><b>งาน</strong></td>
    <td width="60" bgcolor="#999999" class="font_16" style="color:#FFFFFF; padding:2px; "><strong><b>ทั้งหมด </strong></td>
    <td width="50" bgcolor="#999999" class="font_16" style="color:#FFFFFF; padding:2px; "><strong>จ่าย</strong></td>
    <td bgcolor="#999999" class="font_16" style="color:#FFFFFF; padding:2px; "><strong><span class="font_16" style="color:#FFFFFF; padding:2px; "><b>รวม </span></strong></td>
  </tr>
</table>
...
if($_GET[month]=='05'){ 
 ...
 if($_GET[month]>'03' and  $_GET[year]>'2017') { 
 $payplus=1.7;  
} 
 ...
 $payplus=2;  	
  ...
  if($arr[web_driver][company]==1133){
   ...
   if($arr[web_driver][company]==1280){
	 //// Update to 20000 2018-06-09 15.30
	//$pay_bonus=10000;
	$pay_bonus=20000;
	 $payplus=2;  	
  ...
  if($arr[web_driver][company]<>276  and  $arr[web_driver][company]<>283  and  $arr[web_driver][company]<>284 and  $arr[web_driver][company]<>1133 and  $arr[web_driver][company]<>1280  and    $arr[web_driver][company]<>3992  and    $arr[web_driver][company]<>3993  and  $arr[web_driver][company]<>3994  and $arr[web_driver][company]<>3996  ){
...
*/
 ...
 $m_minus = date('Y-m',strtotime("-1 month")); 
...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
//$query = "SELECT SUM(car_price) FROM  ".$dbname."  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' and driver_pickup > '0' and number_of_report >0 and car_price > 0 and drivername='".$_GET[driver]."' group by invoice "; 
$query = "SELECT car_price,invoice FROM  ".$dbname."  where ".$selectdate." like '%" . $no . "%'  and status = 'CONFIRM' and driver_pickup > '0' and number_of_report >0 and car_price > 0 and drivername='".$driver_id."' group by invoice "; 
...
$data_sale="where drivername='".$driver_id."'  $find_date and status = 'CONFIRM'   group by invoice";
 ...
 $data_sale_all="where drivername='".$driver_id."'  $find_date and status = 'CONFIRM'   group by invoice";
  ...
  $data_sale_today_yes="where drivername='".$driver_id."'  $find_date_today and status = 'CONFIRM' and driver_pickup > 0  group by invoice ";



 ;

$data_ot="where drivername='".$driver_id."'  and transfer_date ='". $daywork."' ";
...
//$pay= $row['SUM(car_price)'];
 ...
 $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
$query = "SELECT  car_price  FROM  ".$dbname." $data_sale_today_yes "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
$pay_total_yes = 0;
while($row = mysql_fetch_array($result)){
//$pay= $row['SUM(car_price)'];
  ...
  $pay_total_yes += $row['car_price'];

}


 
if($pay_total_yes>$pay_bonus and $daywork > $paydate){
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
  <tr align="center">
    <td width="50" align="right"  class="font_18" style="height:35px;background-color:<?=$bgcolor ?>; padding-right:20px; font-size:18px; "><? //=$arr[timeline][dayall]?>
...
<div  style="border-radius: 50%; padding:5px; height:30px; width: 30px; background-color:#FF0000;  font-size:18px; color:#FFF  "><center><?=$i?></div> 
...
<? } else { ?>
    ...
    <td width="40"  class="font_18" style="background-color:<?=$bgcolor ?> ">
 
    			<? if($row_yes==0){?>
	<font color="#FF0000"> - </font>
		<? }  else {?>
        
       <a id="#work_<?=$arr[timeline][id]?>" class="check_work_today" day="<?=$arr[timeline][dayall]?>"> 
    ...
    <td width="60" align="right"  class="font_18" style="padding-right:5px;background-color:<?=$bgcolor ?> ">
...
$arr[projectnew] = $db->fetch($res[projectnew]);
  ...
  <div class="modal-dialog"   style="max-width:350px; ">
    ...
    <td width="50" align="right"  class="font_18" style="padding-right: 5px;background-color:<?=$bgcolor ?> "> 
...
<? } ?>
 ...
 <?= number_format( $pay_yes , 0 );?>
    ...
    <td align="right"  class="font_18" style="padding-right:10px;background-color:<?=$bgcolor ?> ">
    
    
    
    <?= number_format( $pay_total_yes , 0 );?>
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
  <tr  >
    ...
    <td width="100" align="right"  class="font_20" style="padding-right: 5px; color:#000000 "> 
 ...
 $('.check_work_today').click(function(){   
 
 var day = $(this).attr("day");
 
 
 
  $( "#load_data_main_popup_acc" ).toggle();
 
 	var url_acc = "load_data_mod_acc.php?name=today&file=work&day="+day;
	
 
	
 
 $('#load_data_main_popup_acc').load(url_acc); 
