...
$pay_all = $db->num_rows(TB_transfer_report_all,"id","drivername='".$driver_id."'  $find_date  and status = 'CONFIRM'  group by  invoice order by driver_checkcar desc ");


$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
 
$query = "SELECT car_price  FROM   transfer_report_all  where drivername='".$driver_id."'  $find_date  and status = 'CONFIRM'  and driver_pickup > 0 group by  invoice "; 
...
///
 ...
 ///$row_all=1;
$daywork_main=$daywork;
 
$string_service     = array ("-");
$string_after_service  = array ("_");
$daywork_main      = str_replace($string_service , $string_after_service ,$daywork_main, $count);   
...
$home_work= $db->num_rows('driver_go_home
',"id","drivername='".$driver_id."'  and transfer_date='".$daywork."' ");
  
 if($home_work==0){
	 
	 
	  $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
 		$db->add_db('driver_go_home',array(
		
 
 "transfer_date"=>$daywork,
 ...
 if($row_all==$row_yes or $pay_all >= 350){
...
$db->update_db('driver_go_home',array( 

 "check_home"=>"1"
		)," drivername='".$driver_id."'  and transfer_date='".$daywork."'  ");
...
$db->update_db(TB_transfer_report_all,array(
		...
		"ondate"=>"2018-07-21",
 		"outdate"=>"2018-07-21",
			 "transfer_date"=>"2018-07-21"
 
		)," idsss='".$arr[project][id]."' ");
...
<span class="font_35" style="font-size:28px; margin-left:15px; width:100%; background-color: #FC9A35; color:#FFFFFF;box-shadow: 1px 1px 5px #999999; border:3px solid #FFFFFF;  border-radius: 40px;"> <a id="btn_load_day_work_<?=$daywork_new?>">   <table width="100%"  border="0" cellspacing="1" cellpadding="1" >
  <tr>
    <td width="100" style="font-size:30px; margin-left:15px;  color:#FFFFFF "><b><? //=$i?><center><? echo $daywork;   ?></b><div style="font-size:20px;  color:#FFFFFF; display:nones "><b>ดาวน์โหลดใบงาน </b>  </div>
 ...
 </center><div id="convert_<?=$daywork_new?>" style="display:none"></div>  
 
 <div id="popup_convert_<?=$daywork_new?>" style="display:none"></div>  
 ...
 $('#btn_load_day_work_<?=$daywork_new?>').click(function(){  
 
 
 $("#popup_convert_<?=$daywork_new?>").show();
 
 
 
 $("#popup_convert_<?=$daywork_new?>").load('go.php?name=today&file=popupwork&day=<?=$daywork?>');
 
 
 
 
  $("#convert_<?=$daywork_new?>").html('<iframe width="100%"  height="100%"  frameborder="0" style="border:0" src="https://www.welovetaxi.com/app/driver_master/data/savework.php?driver=<?=$driver_id?>&day=<?=$daywork?>" allowfullscreen></iframe>');
  
 // $("#close_popup_show_convert_work").hide();
   ...
   ///$("#popup_show_convert_work").show();
  
  
  
 
    setTimeout(function () {
 $("#convert_<?=$daywork_new?>").load('mod/today/loadfile.php?filename=<?=$driver_id?>');
     ...
     setTimeout(function () {
		...
		$("#now_convert_work").hide(); 
   $("#finish_convert_work").show();
   
   $("#close_popup_show_convert_work").show();
			  ...
			  <a  id="btn-all-<? echo $daywork;?>" class="btn btn-app" style="padding:5px; height:35px; width:28%;font-size:16px;border-radius: 15px; background-color:#FFFFFF" >
...
///    alert(55);
 ...
 $('.tab-no-<? echo $daywork;?>').show(); 
			  ...
			  <a  id="btn-no-<? echo $daywork;?>" class="btn btn-app" style="padding:5px; height:35px;width:28%;font-size:16px;border-radius: 15px;			  
...
<div id="main_show_price_<? echo $daywork_new ;?>" style="display:nones">  

 </div>
 
 
 
 <?  if($arr[web_driver][company]==276 or   $arr[web_driver][company]==283 or   $arr[web_driver][company]==284 or $arr[web_driver][company]==3992 or $arr[web_driver][company]==3993 or $arr[web_driver][company]==3994 or $arr[web_driver][company]==3996){ ?>
 
 
 <?  if($row_yes > 6 or $pay_all >= 350){ ?>
<div  style=" font-size:20px; font-weight:bold; background-color:#FFFFFF; border-radius: 0px; color:#367FA9; margin:0px;margin-top:10px;border-radius:25px;border :  2px solid #367FA9; padding:3PX; margin-left:5px; padding:10px;" ><center><i class="fa fa-check-circle"></i>&nbsp;วันนี้นำรถกลับบ้านได้</div>
 <? } else { ?>
 
 <div  style=" font-size:20px; font-weight:bold; background-color:#FFFFFF; border-radius: 0px; color:#FF0000; margin:0px;margin-top:10px;border-radius:25px;border :  2px solid #FF0000; padding:3PX; margin-left:5px; padding:10px;" ><center><i class="fa fa-times-circle"></i>&nbsp;ไม่ให้นำรถกลับบ้าน</div>
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" id="main_edit_work_<?=$arr[project][id];?>" style="border-radius: 15px;" class="tab_alertd">
 ...
 <tr id="tr_confirm_start_work_<?=$arr[project][id];?>" style="display:nones">
      <td><a id="confirm_start_work_<?=$arr[project][id];?>" ><div class="btn-startwork" style="background-color: #3C8DBC;border-radius: 25px; margin:0px; height:40px; padding-top:5px; padding-bottom: 36px;"  ><? //echo $arr[project][airout_time];?><font class="font_22"><center>จัดการงาน</font>
        <? ///=$arr[project][car_number];?>
      </center>
 </div> </a> </td>
    </tr>
     ...
     <tr style="display:nones">
      <td> 
      <div class="btn-startwork" style="background-color: #FFFFFF;border-radius: 25px; margin:0px; color:#333333;  height:auto; padding-top:10px;border :  1px solid #DADADA; font-family:arial "  ><? //echo $arr[project][airout_time];?><font class="font_18"><center><i class="fa fa-clock-o" style="font-size:16px;color:#999999; "></i> <? echo $arr[project][airout_time];?>  &nbsp; <i class="fa fa-calendar" style="font-size:16px;color:#999999 "></i> <? echo $arr[project][outdate];?> </font>
</center>
	  ...
	  <? //=$arr[project] [outdate_time]?>
 </div>
 <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #FFFFFF;border-radius: 25px; margin-top:10px; color:#333333;  height:auto; padding:5px;border :  1px solid #DADADA;  " >
  <tbody>
    <tr>
      <td align="center" style="font-family:arial ; font-size:16px; " id="main_show_nextwork_<? echo $arr[project][id];?>">  
	  </td>
    </tr>
  </tbody>
</table>
...
var url_nexttime_<? echo $arr[project][id];?> = "go.php?name=load/today/sub&file=nextwork&id=<? echo $arr[project][id];?>";
 
 $('#main_show_nextwork_<? echo $arr[project][id];?>').html(load_main_mod_table); 
 
  $.post(url_nexttime_<? echo $arr[project][id];?>,$('#form_popup_send_data').serialize(),

function(response){ $('#main_show_nextwork_<? echo $arr[project][id];?>').html(response); });
 ...
 <div class="btn-startwork bg-green" style="background-color: #FC9A35;border-radius: 25px;margin:0px; height:40px; padding-top:10px; padding-bottom: 36px"  id="change_new_work_<?=$arr[project][id];?>" ><? //echo $arr[project][airout_time];?><center><font class="font_20 "><span  id="change_new_work_text_<?=$arr[project][id];?>">ส่งงานให้คนอื่น</span></font> </div>  
...
$("#timeline_price_<?=$arr[project][id];?>").hide();
 ...
 $('#btn_show_hide_data_<?=$arr[project][id];?>').html('<i class="fa fa-angle-down" style="font-size:24px;  "></i><span style="margin-top:-20px;"></span>');
  ...
  <div  style=" font-size:20px; font-weight:bold; background-color:#FFFFFF; padding:1PX;border-radius: 0px; color:367FA9; margin-top:20px;margin-top:10px;border-radius:25px;border :  1px solid #DADADA; padding:3PX; " >
...
<table width="100%"  border="0" cellspacing="1" cellpadding="1">
  ...
  <div id="show_hide_work_<?=$arr[project][id];?>" style="font-size:20px;     padding:0px;border-radius: 0px; color:333333; padding-left:10px; ">แสดงข้อมูลทั้งหมด</div>
    ...
    <td width="5" align="right" ><div style="font-size:18px; font-weight:bold; background-color:#FFFFFF; padding:5PX;border-radius: 25px; color:367FA9;border :  1px solid #DADADA; width:35px;   "><center>
    
    
    <div id="btn_show_hide_data_<?=$arr[project][id];?>"></div></div>
...
$music_terminal="..อาคารเก่า";
  ...
  <tr>
    ...
    <? if($arr[project][to_place]==544545554193){   ?>
...
<!-- Timeline Remark -->			
			...
			<? if($arr[project][product_area] == 'Out' or $airport=='Out'){  
