...
?>
      ...
      <button style="color: #fffbfb;" class="btnstatus-checkin<? if($arr[project][driver_topoint] > 0){ ?>-no <? } ?> "  data_id="<?=$arr[project][id];?>"  data_vc="<?=$arr[project][invoice];?>"   data_report_id="<?=$arr[project][report_id];?>" col_name="driver_topoint"  id="btn_topoint<?=$arr[project][id];?>" <? if($arr[project][driver_topoint] > 0){ ?> disabled="disabled"  <? } ?> >ถึงสถานที่รับแขก</button>
