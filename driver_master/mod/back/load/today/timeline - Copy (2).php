...
if($_GET[sort]=='all'){
$res[project] = $db->select_query("SELECT $select_order FROM ".TB_transfer_report_all."  where drivername='".$driver_id."'  $find_date and status = 'CONFIRM'  and airout_time <>'' group by invoice order by  outdate,airout_time  ASC ");
}

if($_GET[sort]=='no'){
$res[project] = $db->select_query("SELECT $select_order FROM ".TB_transfer_report_all."  where drivername='".$driver_id."'  $find_date and status = 'CONFIRM'  and airout_time <>'' group by invoice order by  driver_complete,outdate,airout_time ASC ");
}

if($_GET[sort]=='yes'){
$res[project] = $db->select_query("SELECT $select_order FROM ".TB_transfer_report_all."  where drivername='".$driver_id."'  $find_date and status = 'CONFIRM'  and airout_time <>'' group by invoice order by   driver_complete DESC,outdate,airout_time ASC  ");
}
...
<span class="font_35" style="font-size:28px; margin-left:15px; width:100%; background-color: #FC9A35; color:#FFFFFF;box-shadow: 1px 1px 5px #999999; border:3px solid #FFFFFF;  border-radius: 30px;"><table width="100%"  border="0" cellspacing="1" cellpadding="1" >
  <tr>
...
<li class="<? if($arr[project][driver_complete]=="1"){ ?>tab-yes-<? echo $daywork;?><? } ?><? if($arr[project][driver_complete]<>"1"){ ?>tab-no-<? echo $daywork;?><? } ?>"  id="timeline_remark_<?=$arr[project][id]?>">
  <i class="fa fa-comment bg-blue"  style="z-index:1 " id="icon_remark_<?=$arr[project][id]?>"></i>
              <div class="timeline-item" id="div_remark_<?=$arr[project][id]?>" style="margin-left:40px; margin-right: 0px;background-color:FFFFFF;border-radius: 15px;">
                ...
                <h3 class="timeline-header"  style="padding-top:10px; padding-bottom:5px; "><font color="#006699">หมายเหตุ</font></h3>
                <div class="timeline-body"  style="padding-top:5px; padding-bottom:5px; font-size: ">
				<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="td-text">
  ...
  <tbody>
    <tr>
      <td class="td-text"><? echo $arr[project][air];?> / <? echo $arr[project][airin_time];?></td>
            ...
            <li style="margin-top:-15px;"  class="<? if($arr[project][driver_checkcar]=="1"){ ?>tab-yes-<? echo $daywork;?><? } ?><? if($arr[project][driver_checkcar]<>"1"){ ?>tab-no-<? echo $daywork;?><? } ?>"  id="timeline_checkcar_<?=$arr[project][id]?>">
