...
$driver_chat_load->load( '../data/xml/driver/work_to/old/'.$_GET[driver].'.xml' ); 
...
$("#check_data_driver_cancel_work" ).val(1);


if(document.getElementById('check_data_driver_cancel_work_id').value==0){
	
 
 var url_check_data_time_new = "go.php?name=today/check/load/from&file=work_old&report=<?=$report?>&server=<?=$server?>&vc=<?=$vc?>&from=<?=$old_dv?>&to=<?=$new_dv?>&day=<?=$_GET[day]?>";
 
 //$('#check_work_send_popup').show();
 
 //$("#load_driver_cancel_work_show" ).show();
 ...
 $('#check_work_cancel_popup').load(url_check_data_time_new);
   ...
   $("#check_data_driver_cancel_work_id" ).val(<?=$report?>);
