...
$res[project] = $db->select_query("SELECT * FROM car_repair_type  ORDER BY  id  ASC LIMIT $goto, $limit ");
...
?> 
 ...
 <td height="30" align="center">
    ...
    <i class="fa  fa-plus-circle" style="width:40px; font-size:30px;"  ></i>
         ...
         <td align="center"><i class="<? echo $arr[project][icon];?>"  style="font-size:24px"></i></td>
         <td align="center">  
	     <a  onclick="responsiveVoice.speak(' <? echo $arr[project][topic_en];?>', 'UK English Female', {rate: 0.8});" > <? echo $arr[project][topic_th];?></a></td>
         <td align="center">
...
////////////////
		$db->add_db('car_repair_type',array(
      ...
      <tr>
        <td width="100%"><table width="100%" border="0" cellspacing="3" cellpadding="3">
		          <tr>
		            ...
		            <td><input name="name" type="text" class="inputform" id="name" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_th];?>" />
		              </td>
		            </tr>
		          <tr>
                    <td><strong>type :</strong></td>
                    <td><input name="type" type="text" class="inputform" id="type" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][type];?>" /></td>
		            </tr>
		          <tr>
                    <td><strong>icon :</strong></td>
                    <td><input name="icon" type="text" required class="inputform" id="icon" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][icon];?>" />
                    </td>
		            </tr>
...
<?
	...
	if(1==1){
		...
		$db->update_db('car_repair_type',array(
"topic_th"=>"$_POST[name]",
	...
	if(1==1){
		//֧
		$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
		$res[project] = $db->select_query("SELECT * FROM car_repair_type  WHERE id='".$_GET[id]."' ");
...
////echo "<meta http-equiv=refresh content=0;URL=?name=content&file=contact>";
	...
	if(1==1){
		$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
		$db->del('car_repair_type'," id='".$_GET[id]."' "); 
	   ...
	   echo ">".$arr[category][topic_en]." </option>";
		          ...
		          <tr>
                    ...
                    <td><select name="message"   id="message"    style="font-size:14px; padding-bottom: 1px; padding: 5px;  background-color:#FFFDE9; height:30px; width:500px;" chosen width="'100%'">
                        <?  
                      $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
                   $res[category] = $db->select_query("SELECT * FROM  car_repair_type ORDER BY type,topic_en ");
while ($arr[category] = $db->fetch($res[category])){
...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
$res[projecttype] = $db->select_query("SELECT * FROM car_repair_type where   id='".$_GET[id]."'  ");
$arr[projecttype] = $db->fetch($res[projecttype]);
