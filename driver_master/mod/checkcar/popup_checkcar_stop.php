...
<form id="user_car_send_data"  name="user_car_send_data" action="popup.php?name=checkcar/file&file=upload_pic&type=carcare" method="post" enctype="multipart/form-data"  >
 ...
 <div class="topicname"><i class="icon-new-uniF10A-9"></i>&nbsp;รถที่กำลังใช้งาน</div><?
...
?>
  ...
  <tr>
    ...
    <b><?  echo "" . $arr[car][car_num] . "  ( " . $arr[aum][topic_en] . " )  -  " . $arr[admin][company] . ""; ?></b>    
...
<div style="font-size:24px; color:#009999"><?=$arr[chkcar][mile]?></div>
	          ...
	          <input name="check_car_mile_start" type="hidden"  class="form-control" id="check_car_mile_start"  style="font-size:24px; width:100%; border:none; background-color:#FFFFFF" value="<?=$arr[chkcar][mile]?>"  readonly="true" placeholder="ระยะทางที่วิ่ง"/>
...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  <tr>
    ...
    <td width="50%" style="padding-left:10px;"><input name="check_mile_total" type="number"  class="form-control" id="check_mile_total"  style="font-size:20px; width:100%; border:none; background-color:#FFFFFF"  readonly="true" placeholder="ระยะทางที่วิ่ง"/></td>
  </tr>
</table>
 ...
 document.getElementById('check_mile_total').value=parseInt (document.getElementById('check_mile').value)-<?=$arr[chkcar][mile]?>;
                    ...
                    <div class="topicname"><i class="fa  fa-wrench"></i>&nbsp;ตรวจเช็คสภาพรถและล้างรถ</div>
...
// }
 ...
 $('#tb_check_all_car').removeClass("tab_alert");
  ...
  var url_car = "popup.php?name=checkcar&file=savedata&type=usecar&drivername=<?=$driver_id;?>&usecar=stop";
