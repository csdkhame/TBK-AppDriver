...
if($_GET[cartype]==2 ){ 
$res[project] = $db->select_query("SELECT * FROM ".TB_transfer_report_all."  where  id=".$id." ");
...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
	...
	$res[projectshow] = $db->select_query("SELECT * FROM ".TB_transfer_report_all."   where  id=".$id." ");
...
$arr[pickupmap] = $db->fetch($res[pickupmap]);
  ...
  $chk_logo = file_exists("../admin/file/logo/".$arr[project][agent].".jpg?v=<?=time()?>");
