...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
 $allwork = $db->num_rows(TB_transfer_report_all,"id","drivername='".$driver_id."'  $find_date  and status = 'CONFIRM' group by invoice    "); 
...
$res[category] = $db->select_query("SELECT * FROM " . TB_carall . "  where company=276 or company=283 or company=284 order by company,car_num asc");
 ...
 if($arr[category][id] == $arr[car_history][car_number] and $arr[car_history][type]== 'start' ) { 
  $checkdv=1;
          ...
          if ($arr[category][id] == $arr[web_driver][car_num] and $checkdv==0) {
                echo " Selected";
            }
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2" class="drivertable">
  <tr>
    <td width="40%" ><img src="http://t-booking.com/admin/file/driver/pic/<?=$arr[web_driver][post_date];?>.jpg"  width="100%"   border="0"  class="IMGSHOP"   style="margin-top:15px;border-radius:5px;" /></td>
