...
<form method="post" action="" id="edit_form" name="edit_form"  enctype="multipart/form-data" >
                ...
                <!----- ปิด col--> </div> <!---->
                  ...
                  <input type="text" name="line_id" id="line_id"  required="true" onkeypress="PasswordEnter(this,event)"  value="<?=$arr[web_driver][line_id];?>" class="form-control"  >
		  ...
		  <div class="col-md-6">
                    ...
                    <input class="form-control" type="text" name="skype_id" id="skype_id"  required="true" onkeypress="PasswordEnter(this,event)"   value="<?=$arr[web_driver][skype_id];?>" >
               ...
               <input type="text" name="wechat_id" id="wechat_id"  required="true"   value="<?=$arr[web_driver][wechat_id];?>" class="form-control"  >
	...
	<script type="text/javascript">
 			$('#file_upload_line').uploadify({
				'formData'     : {
					...
					'type' : 'line',
					'driver' : '<?=$arr[web_driver][id];?>',
					'name' : '<?=$arr[web_driver][post_date];?>',
					'folder' : 'line_qr' 
				},
  ...
  $('#text_upfile_wechat').html("<font color='#598527'>อัพโหลดไฟล์ QR CODE สำเร็จ</font>");			
	...
	$(function() {
		$('#view_wechat_img').on('click', function() {
			$('.imagepreview').attr('src', "../admin/file/driver/wechat_qr/<?=$arr[web_driver][post_date];?>.jpg");
			$('#imagemodal').modal('show');   
			
					});		
			
					$('#view_line_img').on('click', function() {
			$('.imagepreview_line').attr('src', "../admin/file/driver/line_qr/<?=$arr[web_driver][post_date];?>.jpg");
