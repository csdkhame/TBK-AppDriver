...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
	...
	$res[project] = $db->select_query("SELECT id,name_pickup_place,name_pickup_place_area,name_pickup_place_province,pickup_place FROM ".TB_transfer_report_all."  where id='".$_GET[id]."'   group by invoice");
...
<div style="background-color:#FFF; margin-top:5px; margin-bottom:5px; padding: 2px 0px 2px 0px;border-radius: 20px;"   >
  ...
  include "mod/load/today/sub/place/popup/map_pickup.php";
 ...
 ?>
   ...
   <a id="icon_timeline_show_map_pickup_<? echo $arr[project][id];?>_<?=$_GET[set]?>" >
                      
                                         <img src="images/icon/map/map.png"   align="bottom"  width="30"     /><b>&nbsp;แผนที่นำทาง </a>
                                         
                                         <script>
 
$("#icon_timeline_show_map_pickup_<? echo $arr[project][id];?>_<?=$_GET[set]?>").click(function(){   

 
 
  $("#alert_timeline_show_map_pickup_<? echo $arr[project][id];?>_<?=$_GET[set]?>" ).toggle();
  ...
  <script>
 $("#close_alert_timeline_show_map_pickup_<? echo $arr[project][id];?>_<?=$_GET[set]?>").click(function(){   
 
 $( "#alert_timeline_show_map_pickup_<? echo $arr[project][id];?>_<?=$_GET[set]?>" ).hide();
