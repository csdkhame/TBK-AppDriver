...
$arr[remark] = $db->fetch($res[remark]);		 
  ...
  $db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
$res[remark_noguest] = $db->select_query("SELECT topic_th FROM web_driver_remark WHERE id='".$arr[to][driver_remark_type]."' ");
$arr[remark_noguest] = $db->fetch($res[remark_noguest]);
      ...
      <span style="font-size:  18px; color:#333333"><b> คนขับรถไม่เจอแขก</b></span>
            ...
            <? if($arr[remark_noguest][topic_th]<>''){?>
      ...
      <div style="margin-left:0px; padding:5px;font-size:  16px;"><b>สาเหตุ :</b>  <?=$arr[remark_noguest][topic_th]?>
            ...
            <? if($arr[to][driver_remark_detail]<>''){?>
      
    
      <div style="margin-left:0px; padding:5px;font-size:  16px;"><b>รายละเอียด :</b>  <?=$arr[to][driver_remark_detail]?> </div>
