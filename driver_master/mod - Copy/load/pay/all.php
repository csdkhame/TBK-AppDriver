...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
///$query = "SELECT SUM(park_price) FROM transfer_report_all  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' and driver_pickup > '0' and number_of_report >0 and car_price > 0 and drivername='".$driver_id."' "; 



$query = "SELECT SUM(park_price) FROM transfer_report_all  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' and driver_pickup > '0'   and drivername='".$driver_id."' "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
...
<? if($arr[web_driver][salary_other]>100000){ ?>
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
  <tr  >
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
  <tr  >
    <td  class="font_18" style="height:30px; padding-left:5px; ">เบิกล่วงหน้า (ฝากเก็บเงิน)</td>
    <td width="100" align="right"  class="font_24" style="padding-right:10px; color:#FF0000 "> 
	
	<?= number_format( $pay_deposit, 0 );?>
 ...
 <table width="100%"  border="0" cellpadding="2" cellspacing="2"  style="background-color:#F6F6F6;border:  dotted 1px #999999;  ">
   <tr  >
     ...
     <td width="40%" align="right"  class="font_24" style="padding-right:10px; color: #006699 "><?= number_format( $arr[web_driver][salary]+$payot_all+$pay_park-$arr[web_driver][social_money] -$pay_advance-$pay_deposit, 0 );?>
     </td>
   </tr>
 </table>
