...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
//$query = "SELECT SUM(car_price) FROM  ".$dbname."  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' and driver_pickup > '0' and number_of_report >0 and car_price > 0 and drivername='".$_GET[driver]."' group by invoice "; 
$query = "SELECT car_price,invoice FROM  ".$dbname."  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' and driver_pickup > '0' and number_of_report >0 and car_price > 0 and drivername='".$driver_id."' group by invoice "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
$pay_all = 0 ;
while($row = mysql_fetch_array($result)){
...
///$row_yes = $db->num_rows("".$dbname."","id","drivername='".$driver_id."' $find_date and status = 'CONFIRM' and driver_pickup > '0' and number_of_report >0 and car_price > 0 group by invoice   ");
...
<span  style="border-radius: 20px; padding:5px; height:30px; width: 30px; background-color:#FF0000;  font-size:18px; color:#FFF  "><?=$i?></span> 
