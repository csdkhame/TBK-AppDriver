...
{ 
font-size:22px;   padding:10px;  color:#FFFFF;  width:100%; background-color:#FF0000;      
...
<div class="css-full-popup" id="load_driver_cancel_work_show" style="display:nones; position:fixed; overflow: auto "> 
  ...
  <td align="center"   ><div style="font-size:22px; color:#FFFFFF " id="head_full_popup">ยกเลิกงานโดยฝ่ายจัดรถ</div></td>
 ...
 <script>
   ...
   $("#check_data_driver_cancel_work_id" ).val(<?=$_GET[id]?>);
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
  ...
  <tr>
    <td align="center" class="font_28" style="padding-top:0px; "><b>งานของคุณถูกส่งให้</td>
  </tr>
  <tr>
 ...
 <div id="load_send_work_data_cancel"></div>
 
     
<button type="button" class="btn btn-modal-no"   data-dismiss="modal" data-backdrop="static" id="btn_ok_change_work" style="width:100%; border-radius:20px;" >รับทราบการยกเลิกงาน</button>
...
<?   /// include "load/loading/page_main.php" ; ?>
 ...
 ////  ยืนยันรับงาน
  ...
  $('#btn_ok_change_work').click(function(){   
	   ...
	   swal({
		title: "<font style='font-size:24px'><b>รับทราบการยกเลิกงาน",
 ...
 var url="load_in.php?name=today/action&file=action&action=cancel&driver=<?=$from?>";
 
$('#load_send_work_data_cancel').load(url);

 
 
$("#check_data_driver_cancel_work_id" ).val(0);
$("#check_data_driver_cancel_work_work" ).val(0);


$( "#load_driver_cancel_work_show" ).hide();
