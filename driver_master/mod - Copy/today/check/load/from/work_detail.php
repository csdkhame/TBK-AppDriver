...
if($_GET[cartype]==2 ){ 
$res[project] = $db->select_query("SELECT * FROM ".TB_transfer_report_all."  where  report_id=".$report_id."  and invoice=".$id."");
...
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
	...
	$res[projectshow] = $db->select_query("SELECT * FROM ".TB_transfer_report_all."   where  id=".$ids." ");
...
$res[project] = $db->select_query("SELECT * FROM ".TB_transfer_report_all." where  report_id=".$report_id."  and invoice='".$vc."'");
$allcar="1";
...
if($_GET[admin_company] == 5){
	...
	$transfer_date_admin = $arr[project][transfer_date];
	 ...
	 $db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
	 $res[cartype] = $db->select_query("SELECT * FROM  web_car WHERE id='".$arr[project][cartype]."' ");
...
$pro_name_en_cut= str_replace($string_to , $string_after ,$arr[project][product_name], $count);   
 ...
 <div  style="margin-top:10px; font-size:18px; font-weight:bold; background-color:#F6F6F6; padding:5PX;border-radius: 10px; color:#FF0000"><?=$arr[cartype][topic_th]?> &nbsp;<?=$arr[cartype][pax_th]?> </div>
