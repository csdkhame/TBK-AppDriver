...
if($_GET[query]=="company"){
	if($_GET[admin_com]!=""){
		$admin_com = 'and admin_company = "'.$_GET[admin_com].'" and control_changework=1 ';
	}
	 $db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
    $res[com] = $db->select_query("SELECT company,id FROM web_admin where level=4 and control_transfer=1 ".$admin_com);
...
//	$maincom = 'and  main_company=1';
    ...
    <table width="100%" border="0" cellspacing="2" cellpadding="2" style="<?=$hidden;?>">
  <tbody>
    <tr>
      <td class="font_18"><a href="#" class="mobileSelect-control" id="com_a_<?=$arr[com][id];?>" style="<?=$hidden;?>;font-size: 20px"  data-value="0" onclick="SelectCompany('<?=$arr[com][id];?>');"  > <?=$arr[com][company]?> </a></td>
...
//	$url = "http://t-booking.com/admin/file/driver/pic/small/" . $arr[category][post_date] . ".jpg";
 ...
 <input type="hidden" value="<?=$arr[category][nickname];?>" id="name_<?=$arr[category][id];?>" class="name" />
 	...
 	<tr>
 		...
 		<td   class="td-color other" id="td_<?=$arr[category][id];?>" style="font-size: 18px"><b>
