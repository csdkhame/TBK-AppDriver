...
body,td,th {
 ...
 <div style="padding:5px; margin-top:0px; width:800px;">
...
$res[project] = $db->select_query("SELECT  $select_order  FROM  transfer_report_all where invoice='".$_GET[vc]."' ");
...
$res[web_driver] = $db->select_query("SELECT  * FROM web_driver WHERE id='".$arr[project][drivername]."'    "); 
$arr[web_driver] = $db->fetch($res[web_driver]);
 
////
 

 $res[companydriver] = $db->select_query("SELECT id,company  FROM ".TB_admin." WHERE id='".$arr[web_driver][company]."'    "); 
$arr[companydriver] = $db->fetch($res[companydriver]);
 ...
 $arr[agent] = $db->fetch($res[showlogo]);
...
<table width="100%" border="0" cellspacing="1" cellpadding="1"   id="work_<?=$arr[project][id]?>"  class="div-all-check-today-work">
  <tbody>
    <tr>
      ...
      <td style="font-size:24px; color:#FF0000; background-color:#F6F6F6; padding:5px"><table width="100%" border="0" cellspacing="1" cellpadding="1">
        <tbody>
          <tr>
            <td width="180" style="font-size:28px; color:#000000"><b>
       ...
       <?=$arr[project][invoice]?>
            ...
            <td><table width="100%" border="0" cellpadding="1" cellspacing="1"   id="work_<?=$arr[project][id]?>2" >
              <tbody>
                <tr>
                  <td width="180" style="font-size:24px; color:#FF0000"><? if($arr[project][product_area] == 'In'){ 
						$work_area="รับเข้า";
 ...
 ?>
  ...
  -ms-transform:rotateX(180deg); "></i></td>
   ...
   $work_area="ส่งออก";
                    ...
                    <? } ?></td>
                  <td style="font-size: 22px; "><b>รับแขก&nbsp;<i class="fa  fa-clock-o"  fa-spin 6x style="color:#999999"></i><b>&nbsp;
  ...
  <tr>
    <td width="120" style="font-size: 22px; font-weight:bold;  padding:5px;border-radius: 5px; color:333333 "><? if($arr[project][cartype] == 2){  
	...
	?>
      ...
      <table width="100%"  border="0" cellspacing="0" cellpadding="0" >
        <tr>
          ...
          <td  style="font-size:24px ; font-weight:bold; margin-left:-10px;">จอย </td>
        </tr>
      </table>
      ...
      <table width="100%"  border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td  style="font-size:24px ; font-weight:bold">ไพรเวท</td>
        </tr>
      </table>
    <? } ?></td>
    <td style="font-size: 24px; font-weight:bold;  padding:5px;border-radius: 5px; color:333333 "><b><?=$carpart?>  <?=$arr[project][carno];?>&nbsp;&nbsp;</td>
    <td width="100" align="right" style="font-size:24px; font-weight:bold;  padding:5PX;border-radius: 5px; color:000000 "> <?=$arr[project][invoice];?> </td>
  </tr>
</table></td>
      </tr>
    <tr>
      <td colspan="0" align="center" valign="top" style="font-size:24px; color:#000000"><table width="100%" border="0" cellspacing="4" cellpadding="2">
        <tbody>
          <tr style="display:nones">
              <td colspan="2"   style="color:#000000; font-size:24px; border-bottom: 2px dotted #999999; padding-bottom:10px;"><b>
                    <div><?=$pro_name_th?> <font color="#666666"> <?=$pro_name_en_cut?>
                  </div></td>
            </tr>
          <tr>
            <td width="150" valign="top"><strong>รับที่</strong></td>
            <td valign="top"><b><?=$arr[project][name_pickup_place]?></b> &nbsp;<font color="#333333">&nbsp;(<? echo $arr[project][name_pickup_place_area] ?> | <? echo $arr[project][name_pickup_place_province] ?>)</td>
          </tr>
          
                            <? if($arr[project][product_area] == 'In'){ ?>
          
          <tr>
            <td valign="top"><strong>เที่ยวบิน</strong></td>
            <td valign="top"><b><? echo $arr[project][air];?></b> / <? echo $arr[project][transfer_date];?> เวลา <? echo $arr[project][airin_time];?> น.&nbsp; </td>
          </tr>
          <? } ?>
          ...
          <tr>
            <td valign="top"><strong>ส่งที่</strong></td>
            <td valign="top"><b><?=$arr[project][name_to_place]?></b> &nbsp;<font color="#333333">&nbsp;(<? echo $arr[project][name_to_place_area] ?> | <? echo $arr[project][name_to_place_province] ?>)</td>
          </tr>
 	 ...
 	 if($arr[project][adult]>0){ ?>
          ...
          <tr>
            <td><strong>เบอร์โทรแขก</strong></td>
            <td ><?=$arr[project][guest_phone]?></td>
          </tr>
       ...
       <? } ?>   
          ...
          <tr>
            ...
            <? if($arr[project][car_price]>0){
				$arr[project][car_price]=$arr[project][car_price];
			}
			else {
          ...
          <tr>
            <td><strong><br>
              ค่าเที่ยว</strong></td>
            <td style="font-size:22px;"><br>
              <b><?=$arr[project][car_price]?></td>
          </tr>
          <tr>
            <td><strong>เอเย่นต์</strong></td>
            <td  ><?=$arr[agent][company]?></td>
          </tr>
          <tr>
            <td><strong>บริษัทรถ</strong></td>
            <td  ><?=$arr[companydriver][company]?></td>
          </tr>
