...
while ($arr[category] = $db->fetch($res[category])){
    ...
    $arr[company] = $db->fetch($res[company]);
	   ...
	   echo ">".$arr[category][username]."&nbsp;".$arr[category][nickname]."  (".$arr[company][company].")</option>";
	   }
                      $db->closedb ();
                      ?>
  </select>
...
<div  style="margin-top:-5px; font-size:22px; font-weight:bold;   padding:10px; margin-right:18px; padding-bottom:1px;  width:100%; ">
...
$res[work] = $db->select_query("SELECT * FROM transfer_report_all   where  admin_company =1   and transfer_date='". $daywork."'  and status = 'CONFIRM'  and airout_time <>'' and  carno >0 and drivername > 0  $findpart  group  by  carno order by  carno,airout_time ASC    ");
...
while ($arr[work] = $db->fetch($res[work])){
  ...
  if($arr[work][agent]=='13' and  $arr[work][cartype] <>'2'){
			...
			$carpart="Join";			
				...
				*/  
	   echo "<option value=\"".$arr[work][carno]."\"";
...
$('#data_popup_<?=$arr[project][id];?>').load(url_all);
	 ...
	 document.getElementById('carno<?=$arr[project][id];?>').value = <? echo $arr[project][carno];?>;
  ...
  $("#btn_confirm_change_<?=$arr[project][id];?>").click(function(){
	...
	url=url+"&carno="+document.getElementById('carno<? echo $arr[project][id];?>').value;
