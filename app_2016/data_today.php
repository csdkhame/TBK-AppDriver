...
$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
$res[project] = $db->select_query("SELECT * FROM ".TB_order."  where drivername='".$driver_id."' and transfer_date='".$_GET[day]."' and status = 'CONFIRM'  group by carno   order by airout_time ASC   ");
                  ...
                  <table align="center">
                     <tbody>
                        <tr>
                           <td colspan="2" >
                           <div>
						   <img src="../icon/flag/32/Thailand.png" align="absmiddle"/>
...
<li class="dx" mlevel="<?=$i;?>" <? if($arr[project][driver_complete] == 1){ ?> style="background-color : #b3f2d9;"  <? } ?>>
	<a rel="external" href="show_today.php?day=<?=$_GET[day];?>&carno=<?=$arr[project][carno];?>&car_list=<?=$i;?>&sv=TH">
                  <table>
                     <tbody>
                        <tr>
                           <td  >
