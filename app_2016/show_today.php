...
while($arr[project] = $db->fetch($res[project])){
  ...
  echo $chk_logo = file_exists("../admin/file/logo/".$arr[project][agent].".jpg");
...
<?
$res[remark_noguest] = $db->select_query("SELECT * FROM web_driver_remark_noguest WHERE status='1' ");
...
<? } ?>	
	...
	<tr>
		<td>อื่น ๆ :</td>
		<td> <input type="text" style="width: 200px;" name="driver_remark_other" id="driver_remark_other"/></td>
	</tr>
</table>

</div> 

<div id="dialog_remark_topoint" title="สาเหตุที่ถึงสถานที่รับสาย" style="display:none;padding:30px 10px;">
<table>
	<tr>
		<td width="100">สาเหตุ :</td>
		<td>
	...
	<option value="0">---- กรุณาเลือกสาเหตุที่ไม่เจอแขก ----</option>
<?
$res[remark_noguest] = $db->select_query("SELECT * FROM web_driver_remark_topoint WHERE status='1' ");
while($arr[remark_noguest] = $db->fetch($res[remark_noguest])){
?>	
	<option value="<?=$arr[remark_noguest][id];?>"><?=$arr[remark_noguest][topic_th];?></option>
<? } ?>	
